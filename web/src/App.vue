...
<template>
  <div class="min-h-screen">
    ...
    <main class="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      <section v-if="finishedSessions.length" class="mt-10"><div class="mb-4 flex items-center gap-2"><BellRing :size="19" class="text-coral" /><h2 class="text-lg font-bold">Tempos encerrados</h2><span class="rounded-full bg-pink-soft px-2 py-0.5 text-xs font-bold text-pink-deep">{{ finishedSessions.length }}</span></div><div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><SessionCard v-for="session in finishedSessions" :key="session.id" :session="session" :now="now" @paid="markPaid" @remove="removeSession" @reset="resetSession" /></div></section>
    </main>
